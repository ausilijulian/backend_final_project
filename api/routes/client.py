...
def create_client(id_user):
    ...
    if not all(isinstance(request_data[key], str) for key in ['name', 'surname', 'email', 'dni']):
        return jsonify({"error": "Invalid data types for name, surname, email, or dni"}), 400
    ...
    cur.execute('SELECT * FROM client WHERE email = %s AND id_user = %s AND deleted = 1', (email, id_user)) 
    row = cur.fetchone()                                           
    if row: 
    ...
    row = cur.fetchone() 
    return jsonify({"id": row[0], "name": name, "surname": surname, "email": email, "dni": dni, "id_user": id_user }),201
...
def update_client(id_client,id_user):
    ...
    #control si existe el email PERO no del cliente que se edita
    #(esto  tambien permite editar otros campos de client sin se que se bloquee el UPDATE porque el cliente ya tiene su email registrado)
    ...
    mysql.connection.commit() #guardado
    return jsonify({"id":id_client, "name": name, "surname": surname, "dni": dni, "email": email}),201
...
def remove_client(id_client,id_user):
    ...
    cur = mysql.connection.cursor()
    result = cur.execute('UPDATE client SET deleted = 0 WHERE id = %s', (id_client,))
    
    if result == 0:
        # Si ninguna fila fue afectada, el cliente no fue encontrado
        cur.close()
    ...
    return jsonify({"message": "Client deleted", "id": id_client}), 200